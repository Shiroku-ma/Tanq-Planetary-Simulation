...
function nBody(N, mass, pos, vel, tEnd, dt)
    ...
    println("Initial mass of sun: ", mass[1])
    ...
    println("Final mass of sun: ", mass[1])
...
function main()
    ...
    tEnd = 60.0 * 60.0 * 24.0 * 2000 #Endtime
    dt = 60.0 * 60.0 #Delta time
    poses_nbody_save = nBody(N, mass, pos, vel, tEnd, dt)

    # 4.Plot
    p1 = plotAll(N, poses_nbody_save, tEnd, dt, 8.0, "Planet Orbits")
    scatter_interval = 864000
    p2, anim2 = scatterAll(N, poses_nbody_save, tEnd, dt, 8.0, scatter_interval, true, "Planet Motinos")
    savefig(p1, "$N-$tEnd-$dt-$scatter_interval-1.png")
    savefig(p2, "$N-$tEnd-$dt-$scatter_interval-2.png")
    gif(anim2, fps=10, "$N-$tEnd-$dt-$scatter_interval.gif")
end
